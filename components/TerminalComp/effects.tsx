...
export const MatrixRain: React.FC = () => {
  ...
  const [columnDelays, setColumnDelays] = useState<number[]>([]);
  const [columnChars, setColumnChars] = useState<string[][]>([]);
  const [isMounted, setIsMounted] = useState(false);

  useEffect(() => {
    const nextColumns = window.innerWidth < 768 ? 20 : 50;
    setColumns(nextColumns);
    setColumnDelays(Array.from({ length: nextColumns }, () => Math.random() * 5));
    setColumnChars(
      Array.from({ length: nextColumns }, () =>
